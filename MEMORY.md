...
- [LLM-CONFIG.md](docs/tech-design/LLM-CONFIG.md) — LiteLLM 配置、模型路由、Fallback、Cost 追踪、LLM usage EvalEvent
...
- [AI-RUNTIME-CONVERGENCE.md](docs/architecture/AI-RUNTIME-CONVERGENCE.md) — AI Runtime 收敛主线：Project → Intent → Pipeline → Stage → StageExecutionContext → Agent/Profile → Skill Runtime → Artifact → Delivery → Eval Feedback，作为 TASK-028～TASK-047 的实现基线和 TASK-048～TASK-053 的工作流增强入口
...
- TASK-047 已完成：`StageDefinition` 新增必需输入和完成标准，`StageExecutionContext` 按 Project/Run/阶段隔离并按阶段类型选最新 Artifact，执行上下文受 6 项/单项 4000/总计 12000 字符预算约束，Artifact 正文以不可信 user-level reference 注入；完成归档异常会将阶段置为 failed。TASK-048 下一步优先修复 Dashboard Task 统计、费用和最近任务的多租户隔离风险，随后推进 TaskGraph、WorkspaceExecutor、VerificationGate、PipelineOrchestrator 和全链路 E2E。
