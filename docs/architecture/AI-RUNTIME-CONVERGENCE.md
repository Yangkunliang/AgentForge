...
- 被 StagePolicy 过滤但已绑定到当前 Agent 的高风险 Skill 会通过 `skill_authorization_required` SSE 通知前端，Chat 授权卡可确认后重试。
...
- TASK-032 已将高风险权限确认接入 GovernancePolicy，并把拒绝审计写入 `governance_decision`；TASK-035/TASK-039 已让内置 Skill、外部 Skill、MCP tool、临时授权和前端授权重试进入 Stage 级可见工具链路。
...
目标：让内置 Skill 和外部 Skill/MCP 一样进入 RuntimeSpec、StageSkillPolicy 和高风险权限语义。

完成状态：已落地 `external_side_effect` 高风险权限、内置 Skill RuntimeSpec 生成，以及默认 StagePolicy 对 `http_request`、`update_profile`、`code_executor` 的过滤测试。
...
目标：把 TASK-038 的运行时授权契约接到 Chat UI，让用户能看到并确认当前阶段需要的高风险 Skill。

完成状态：已落地 `skill_authorization_required` SSE、`ChatRequest.skill_authorization`、前端 `SkillAuthorizationCard` 和授权后重试原消息。

不做：

- 不改造为长期审批流。
- 不让 `agent_not_allowed` 的 Skill 出现在授权卡片里。

### TASK-040: 高风险 Skill 授权可观测性

目标：把高风险 Skill 的授权请求和授权使用变成可查询的 EvalEvent，服务后续 AgentProfile、SkillPolicy 和默认权限策略优化。

完成状态：`SkillToolFilterReport` 已记录 `authorized_tools`；StageRuntime 会写入 `skill_authorization_required` 和 `skill_authorization_granted` EvalEvent。
...
### TASK-046: Dashboard LLM 成本与用量

目标：让平台用户在 Dashboard 直接观察当前账号的非流式 LLM 调用、成本、Token、延迟和主要消耗来源。

完成状态：StageRuntime 与 SkillExecutionEngine 已统一使用 `evaluation_context`；Evaluation summary 增加仅基于 `llm_*` 的 ModelRoute / Stage 聚合；Dashboard API 和页面展示总览与前 3 成本排行。

不做：

- 不采集 `stream_complete` usage。
- 不启用 `budget_policy` 确认或拦截。
- 不新增成本聚合表或外部 BI 依赖。

### TASK-047～TASK-053: 核心工作流执行链

目标：把现有状态主链路升级为语义和执行都可验证的工程闭环：
...
- 上游 Artifact 正文属于不可信数据，不进入 system prompt 的可信指令区。
- WorkspaceExecutor 只能操作用户主动授权的 ProjectMount。
- VerificationGate 必须依据真实命令结果，失败不能伪装成已完成。
- PipelineOrchestrator 只推进状态，不承载具体 Skill 或文件修改逻辑。
...
| 长期优化无数据 | EvalEvent 已记录阶段、Skill、交付、确认、高风险授权、失败事实和非流式 LLM 成本事实，Artifact metadata 已固化生成来源，Evaluation summary 和 Dashboard 已聚合高风险授权及 LLM 指标；流式 LLM usage 可继续增强 | 后续增强 |
