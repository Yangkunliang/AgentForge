...
### 7.3 WorkspaceExecutor 多文件写入

TASK-050 后，WorkspaceExecutor 可把 TaskNode 的多文件 Patch 应用到授权工作区。它不是 LLM 可任意调用的 filesystem/shell 工具，必须满足：

- 只接受当前用户 Project 下、与 TaskGraph/TaskNode 同 Project 的 connected local primary Mount。
- Preview 封存规范化 Mount root 的 sha256；Apply 时 root、Mount 状态或角色变化均视为授权已撤销并拒绝写入。
- 每个路径必须同时存在于 TaskNode.target_files，并复用 Bridge 的相对路径、授权根、符号链接逃逸和敏感文件检查。
- Preview 只持久化候选正文、unified diff 和基线指纹，不写工作区；API 不返回 `proposed_content`。
- Apply 必须显式传入 `confirm_write=true`，GovernancePolicy 记录 `confirmation_type=workspace_write` 和完整影响路径。
- Apply 使用 WorkspaceChangeSet 数据库行锁；任何文件写入前先校验全部基线，写每个文件前再次校验，冲突时拒绝覆盖。
- 正常写入异常会按相反顺序恢复已尝试文件；已存在文件恢复原正文，新文件移除，并保存不含源码的 ApplyReport。
- applied 版本幂等，failed 版本不得重试，必须重新 Preview 获取新基线。
- `workspace.preview.*`、`workspace.apply.denied/conflict/failed/succeeded/idempotent` 写入 `AuditLog.resource=workspace_change_set`；details 不记录 proposed content、原文件正文或 diff。
- `applying` 是跨数据库和本地文件系统的恢复日志；进程崩溃无法依赖单一 ACID 事务自动回滚，后续恢复编排必须识别并人工或自动核对基线。

### 7.4 Artifact Delivery 本地写回
...
### 7.5 Artifact Delivery GitHub PR
...
### 7.6 Artifact Delivery zip Package
...
| `workspace.preview.*` | INFO/WARN | WorkspaceChangeSet 预览成功或失败 |
| `workspace.apply.denied` | WARN | WorkspaceChangeSet 未确认被拒绝 |
| `workspace.apply.conflict` | WARN | Preview 后工作区基线发生变化 |
| `workspace.apply.failed` | ERROR | 文件写入或正常回滚路径失败 |
| `workspace.apply.succeeded` | INFO | 多文件 Patch 已应用到授权 Mount |
