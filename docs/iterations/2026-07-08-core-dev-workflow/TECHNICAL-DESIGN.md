...
- `/projects/create` 先创建 Project，再创建 primary Mount；TASK-018 后 connected local Mount 可用于 ContextPicker 文件选择。
...
TASK-017 已落地：`confirm` 支持 `approve`、`revise`、`cancel`；`revise.feedback` 会写入 `confirmation_feedback` 并注入下一次同阶段执行上下文，确认操作写入 `AuditLog`。
...
- `agentforge mount <path>` 创建 connected local ProjectMount，并记录 `metadata.root_path`。
- `agent_forge.bridge.files` 负责授权根目录解析、路径穿越拒绝、敏感文件拒绝、目录过滤和 UTF-8 文本读取。
- `ContextPickerDialog` 只展示当前项目 connected local Mount 的文件；同名文件以 `MountName/完整相对路径` 显示。
- Chat 请求的 `context_files[type=file].mount_id` 会触发后端读取真实文件内容，并注入 SkillExecutionEngine system prompt。
- 未带 `mount_id` 的 file 上下文仍只是线索，不代表已读取。
...
- `DeliveryService` 使用 Artifact 内容和 Mount 目标文件生成 unified diff。
- `preview` 不写文件；`apply` 必须显式 `confirm_write=true`。
- 写回复用 `agent_forge.bridge.files` 的授权 root、相对路径和敏感文件拒绝规则。
- 目标文件存在时写入前生成 `.agentforge.bak` 备份，并将 `backup_path`、`bytes_written` 等写入 `Artifact.delivery_report`。
- Artifact Viewer 提供交付面板，支持 diff 预览、确认写回、交付报告展示和 Markdown 导出。
- TASK-020 已加固：preview report 包含目标文件 fingerprint；apply 可传 `expected_target_hash`，目标文件在预览后变化时返回 409，Artifact 标记为 `failed` 并保存失败报告；成功、拒绝、冲突和失败都会写入 `AuditLog.resource=artifact_delivery`。
...
- TASK-017 已实现：根据阶段规则判断是否需要确认，进入 `waiting_confirmation`，创建待确认 Artifact，发出 `confirm_required`，并在确认前停止自动推进。
- TASK-018 已实现：执行当前阶段前可加载用户选中的授权 Mount 文件内容和历史 Artifact 线索。
- TASK-019 已实现：Artifact Viewer 可对 Artifact 内容生成 diff，并在用户确认后通过 DeliveryService 写回授权 Mount。
...
| 确认节点只是 UI 按钮 | Agent 可能绕过 PRD/技术/影响范围确认 | TASK-017 将确认做成 StageState 状态和 API |
| Bridge 过早接入 | 范围过大，拖慢底座 | TASK-018 延后 |
| Delivery 直接写文件 | 误写用户本地项目，恢复困难 | TASK-019 使用 diff preview、`confirm_write`、授权 root 校验和写前备份；TASK-020 增加 expected hash、失败报告和审计 |
