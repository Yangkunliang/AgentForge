...
POST /api/v1/artifacts/{artifact_id}/use-as-context
```
...
- 根据 intent 创建阶段列表。
- 执行当前阶段前加载 Project、Mount、active context 和历史 Artifact。
- 调用 SkillExecutionEngine 执行阶段任务。
- 将阶段输出保存为 Artifact。
- 根据阶段规则判断是否需要确认。
- 推送 SSE 事件更新前端。
...
| Project 仍是 mock | 多项目用户无法使用 | TASK-013/014 优先 |
