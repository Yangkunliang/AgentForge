...
已补充 `web/e2e/execution-steps.spec.ts` 和 dev-only harness route `/__e2e__/execution-steps`，用 Chromium 覆盖：

- 纯对话不渲染空执行步骤。
- thinking → weather tool_call → code_execution 顺序展示。
- 历史 `tool_calls` 兜底渲染。
- SSE 中断后无 spinner/进度条残留。
- streaming 场景下执行过程自动展开并显示进度反馈。
- 折叠/展开动画 CSS duration 不超过 300ms。
- 多步骤时间线 connector 横向对齐。
- 375px 移动端无横向溢出。

真实 LLM Provider 和第三方天气服务网络链路不在本轮视觉验收范围内，后续可在完整端到端环境中单独抽验。
